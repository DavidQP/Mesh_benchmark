class Boid {
  ...
  Boid(Vector inPos, ArrayList<PShape> fv , ArrayList<PVector[]> vv, PShape vertex ) {
  ...
  void render() {
    ...
    if(key == 'g'){
      ...
      println("retained ");
      println("Vertex vertex " + frameRate);
      //vertex vertex
      shape(vertex);
    } 
    ...
    //FaceVertex
      ...
      for(int iter=0; iter<fv.size() ; iter++){
        
        shape(fv.get(iter));
        /*for(int i = 0; i < vv.size(); i++){
          PVector[] arr = vv.get(i);
          for(int j = 0; j < arr.length; j++){
            vertex(arr[j].x,arr[j].y,arr[j].z);
          }
        }*/
